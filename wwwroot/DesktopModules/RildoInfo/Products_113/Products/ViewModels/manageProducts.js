...
$(function () {
    ...
    $("#productsGrid").kendoGrid({
        ...
        columns: [
            ...
            {
                field: "ProductRef", title: "Referência", width: 85, hidden: true
            },
            {
        ...
        dataBound: function () {
            ...
            $('#btnExportProducts').attr({ 'disabled': true });
    ...
    $('#btnSyncProducts').click(function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: '/desktopmodules/riw/api/products/SyncProducts?sDate=' + moment(new Date(2015, 5, 1)).format('YYYY-MM-DD') + '&eDate=' + moment(new Date()).add(1, 'days').format('YYYY-MM-DD')
        }).done(function (data) {
            if (data.Result.indexOf("success") !== -1) {
                $.pnotify({
                    title: 'Sucesso!',
