...
$(function () {
    ...
    my.loadEstimate = function () {
        ...
        if (my.estimateId > 0) {
            $.getJSON('/desktopmodules/riw/api/estimates/getEstimate?estimateId=' + my.estimateId + '&portalId=' + portalID + '&getAll=true', function (estimateData) {
    ...
    my.addEstimateItem = function () {
        ...
        $.ajax({
            ...
            if (data.Result.indexOf("success") !== -1) {
                ...
                if (my.vm.estimateId() === 0) {
                    my.vm.estimateId(data.EstimateItem.EstimateId);
                    history.pushState("", document.title, window.location.pathname);
                    document.location.hash = 'estimateId/' + data.EstimateItem.EstimateId + '/personId/' + (my.vm.personId() > 0 ? my.vm.personId() : mainConsumerId);
                }
                ...
                my.vm.prodTotal((my.vm.itemQty().length > 0 ? my.vm.itemQty() : '1') + ' x ' + my.vm.unitValue() + ' = ' + kendo.toString(Math.floor((my.vm.itemQty().length > 0 ? kendo.parseFloat(my.vm.itemQty().replace('.', ',')) : 1) * my.vm.unitValue() * 100) / 100, 'n'));
    ...
    my.itemsSearchBox.kendoAutoComplete({
        ...
        template: '<strong>Produto: </strong><span>${ data.ProductName }</span><br /><strong>Ref: </strong><span>${ data.ProductRef }</span> <div class="pull-right"><strong>Estoque: </strong> <span>${ data.QtyStockSet }</span></div><strong>Cod. Barra: </strong><span>${ data.Barcode }</span><br /><strong>Valor: </strong><span> #= kendo.toString(data.UnitValue, "c") #</span>',
        dataSource: {
            transport: {
                read: '/desktopmodules/riw/api/products/GetProducts?portalId=' + portalID + '&pageIndex=1&pageSize=1000&orderBy=ProductName'
            },
            ...
            switch (true) {
                ...
                case (this.dataSource.total() === 1):
                    ...
                    if (dataItem) {
                        ...
                        //my.vm.itemQty($('#itemQtyBox').val().length > 0 ? $('#itemQtyBox').val() : '1');
                        //my.vm.unitTitle(dataItem.UnitTypeTitle);
                        //my.vm.prodImageId(dataItem.ProductImageId);
                        //my.vm.unitValue(dataItem.UnitValue);
                        ...
                        //        } else {
                        //            return '<div class="view view-first"><img alt="' + dataItem.ProductName + '" src="http://a1.cambelt.co/icon/camera/130" /></div>';
                        //        }
                        //    }).click(function (e) {
                        //        window.open('/produtos?SkinSrc=[G]Skins/riw/popUpSkin&ContainerSrc=[G]Containers/riw/popUpContainer#itemid/' + dataItem.ProductId, 'PrintMe', 'toolbar=no,menubar=no,location=no,width=720,height=580');
                        //        return false;
                        //    });
                        //} else {
                        //    $('#divImage').html(function () {
                        ...
                        if (dataItem.ProductsRelatedCount > 0) {
                            ...
                            my.vm.summary(dataItem.Summary);
                            //my.vm.prodDesc(dataItem.Description);
                            my.vm.itemQty(my.vm.itemQty().length > 0 ? my.vm.itemQty() : '1');
                    ...
                    buttons: {
                        'ok': {
                            ...
                            click: function (e) {
                                ...
                                my.vm.items.remove(function (item) { return item.EstimateItemId === selectedItem.EstimateItemId; });

                                var estimateItems = [];
                                estimateItems.push({
                                    EstimateId: my.vm.estimateId(),
                                    EstimateItemId: selectedItem.EstimateItemId,
                                    RemoveReasonId: $('#selectDeleteReason').data('kendoDropDownList').value(),
                                    ProductId: selectedItem.ProductId,
                                    ProductName: selectedItem.ProductName,
                                    ProductQty: selectedItem.ProductQty,
                                    CreatedByUser: userID,
                                    CreatedOnDate: moment().format()
                                });
                                ...
                                $.ajax({
                                    ...
                                    if (data.Result.indexOf("success") !== -1) {
                                        ...
                                        self.productName(slogan);
                                        self.prodTotal('0 x 0 = 0,00');
                                        my.lineTotal = my.lineTotal - (kendo.parseFloat(selectedItem.ProductQty) * selectedItem.UnitValue);
    ...
    $('#btnPrintReceipt').click(function (e) {
        ...
        var newWindow = window.open('', '_blank');
        var locationStr = 'http://' + siteURL + '/portals/0/downloads/recibo_' + my.estimateId + '_' + moment().format('DD-MM-YYYY HH').replace(':', '-').replace(' ', '_') + '.txt';
    ...
    $('#btnPrintTxt').click(function (e) {
        ...
        $.ajax({
            ...
            //    xhr.setRequestHeader("Accept", "application/octet-stream");
            //},
            url: '/desktopmodules/riw/api/estimates/downloadEstimateTxt',
            data: {
                PortalId: portalID,
                EstimateId: my.estimateId,
                PersonId: my.personId > 0 ? my.personId : my.getParameterByName('personId'),
            ...
            if (data.Result.indexOf("success") !== -1) {
                setTimeout(function () {
                    ...
                    newWindow.location = 'http://' + siteURL + '/portals/0/downloads/orcamento_' + my.estimateId + '_' + moment().format('DD-MM-YYYY HH').replace(':', '-').replace(' ', '_') + '.txt';
        ...
        setTimeout(function () {
            switch (true) {
                ...
                case (my.vm.selectedPayForm().toString().indexOf('Dinheiro') !== -1):
                    ...
                    break;
                case (my.vm.selectedPayForm().toString().indexOf('bito') !== -1):
                    $('#cardInTextBox').focus().select();
                    break;
                default:
                    $('#cashInTextBox').focus().select();
        ...
        $.ajax({
            ...
            if (data.Result.indexOf("success") !== -1) {
                ...
                kendoWindow.data("kendoWindow")
                    .content('<h3 class="DNNAligncenter">DAV ' + my.estimateId.toString() +
                             '</h3><h5 class="DNNAligncenter"><a href="/portals/0/davs/DV' +
                             my.padLeft(my.estimateId.toString(), 6) + '.txt">' +
                             my.estimateId.toString() +
    ...
    $('#btnCloseDiscount').click(function (e) {
        e.preventDefault();
    ...
    $('#btnApplyDiscount').click(function (e) {
        ...
        var estimateItems = [];
        if (my.vm.estimateItemId()) {
            estimateItems.push({
                EstimateId: my.vm.estimateId(),
                EstimateItemId: my.vm.estimateItemId(),
                ProductQty: my.vm.newItemQty(),
                ProductEstimatePrice: kendo.parseFloat(newItemPrice), // my.vm.itemPrice()
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                ...
                TotalAmount: originalAmount = originalAmount - (originalAmount * my.vm.couponDiscount() / 100),
                ModifiedByUser: userID,
                ModifiedOnDate: moment().format(),
    ...
    my.statusDropDown = $('#ddlStatuses').data("kendoDropDownList");

    my.statusDropDown.bind('select', function (e) {
        var dataItem = this.dataItem(e.item.index());
    ...
    var SessionManager = function () {
        ...
        //        You can just hard-code your timeout here if you feel like it.  But I might cry. ?smt=30&smc=20
        var sessionTimeoutSeconds = my.htmlHelpers.getQueryStringValue('smt') || (40 * 60),
            ...
            extendSessionUrl = '/desktopmodules/riw/api/store/ExtendTime',
            expireSessionUrl = '/desktopmodules/riw/api/store/ExpireTime';
        ...
        var endSession = function () {
            ...
            $.get(expireSessionUrl, { portalId: portalID, userName: userName }, function (data) { });
            setTimeout(function () {
                location.href = returnURL;
            }, 500);
        };
        ...
        var displayCountdown = function () {
            var countdown = function () {
                ...
                document.title = 'Expira em ' + my.stringHelpers.padLeft(minutes, '00') + ':' + my.stringHelpers.padLeft(seconds, '00');
        ...
        var promptToExtendSession = function () {
            ...
            kendoWindow.data("kendoWindow")
                .content($("#sm-countdown-dialog").html())
                .center().open();

            kendoWindow.find(".confirm-continue,.confirm-exit").click(function () {
                if ($(this).hasClass("confirm-continue")) {
                    refreshSession();
