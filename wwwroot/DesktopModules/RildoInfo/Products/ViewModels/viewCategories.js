...
$(function () {
    ...
    // apply ko bindings
    ko.applyBindings(my.categoryViewModel, document.getElementById('dnn_ctr434_ModuleContent'));

    // ajax call to get logged in user's roles and add them to ko view model roles
    my.loadRoles = function () {
        $.ajax({
            url: '/desktopmodules/riw/api/store/GetRolesByUser?portalId=' + portalID + '&uId=' + userID,
            async: false
        }).done(function (data) {
            if (data) {
    ...
    my.loadCategories = function () {
        ...
        if (my.categoryViewModel.categories().length === 0) {
            $.ajax({
                async: false,
                url: '/desktopmodules/riw/api/categories/Categories?portalId=' + portalID + '&lang=pt-BR&includeArchived=false'
            }).done(function (data) {
