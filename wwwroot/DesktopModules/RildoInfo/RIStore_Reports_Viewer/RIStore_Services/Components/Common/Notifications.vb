...
#Region "Subs and Functions"
        ...
        Friend Shared Sub EstimateNotification(_contentTypeName As Constants.ContentTypeName, _notificationTypeName As Constants.NotificationEstimateTypeName, _senderId As Integer, _recipient As Users.UserInfo, _role As String, _portalId As Integer, _estimateId As Integer, _subject As String, _body As String)

            Dim notificationType As NotificationType = NotificationsController.Instance.GetNotificationType(_contentTypeName.ToString())
        ...
        Friend Shared Sub RemoveEstimateEntryNotification(_contentTypeName As Constants.ContentTypeName, _notificationTypeName As Constants.NotificationEstimateTypeName, ByVal _estimateId As Integer, _userId As Integer)

            Dim notificationType = NotificationsController.Instance.GetNotificationType(_contentTypeName.ToString())
        ...
        Public Shared Sub SendStoreEmail(fromUser As Users.UserInfo, toUser As Users.UserInfo, cctoUser As Users.UserInfo, bccToUser As Users.UserInfo, _subject As String, _body As String, _fileAttach As IO.Stream, _fileType As String, _fileIdAttach As Integer, _bodyType As DotNetNuke.Services.Mail.MailFormat, _priority As DotNetNuke.Services.Mail.MailPriority, sendUser As Users.UserInfo, _portalId As Integer, useStoreSMTP As Boolean)
            ...
            Using objEmail As New SendEmail() With {.Body = _body, .Subject = _subject, .BodyFormat = _bodyType, .Priority = _priority}
                ...
                If useStoreSMTP Then
                    ...
                    _smtpServer = myPortalSettings("RIS_SMTPServer")
                    _smtpPort = CInt(myPortalSettings("RIS_SMTPPort"))
                    _authType = "1"
                    _smtpLogin = myPortalSettings("RIS_SMTPLogin")
                    _smtpPassword = myPortalSettings("RIS_SMTPPassword")
                    _smtpSSL = CBool(myPortalSettings("RIS_SMTPConnection"))
                End If
...
#Region "Install"
        ...
        Friend Shared Sub AddNotificationTypes()
            ...
            Dim deskModuleId As Integer = DesktopModuleController.GetDesktopModuleByFriendlyName("RIStore Estimate").DesktopModuleID

            Dim contentType = Constants.ContentTypeName.RIStore_Estimate
            Dim objNotificationType As NotificationType = New NotificationType() With {
                .Name = contentType.ToString(),
                .Description = "RIStore Estimates",
            ...
            If NotificationsController.Instance.GetNotificationType(objNotificationType.Name) Is Nothing Then
                ...
                'objAction.APICall = "/DesktopModules/RIStore/API/RIStoreService.ashx/OpenNewEstimate"
                'objAction.Order = 1
