...
#End Region
        ...
        Public Function GetPayemnt(paymentId As Integer, portalId As Integer) As Models.Payment Implements IPaymentsRepository.GetPayemnt
            ...
            Using ctx As IDataContext = DataContext.Instance()
                product = ctx.ExecuteQuery(Of Models.Payment)(CommandType.StoredProcedure, "RIS_Payment_Get", paymentId)
            End Using
            Return product
        End Function

        Public Function GetPayments(ByVal portalId As String, ByVal account As String, ByVal provider As String, ByVal client As String, ByVal cat As String, ByVal done As Boolean, ByVal sTerm As String, ByVal sDate As String, ByVal eDate As String, ByVal pageNumber As Integer, ByVal pageSize As Integer, orderBy As String) As IEnumerable(Of Models.Payment) Implements IPaymentsRepository.GetPayments
            Dim payments As IEnumerable(Of Models.Payment)

            Using ctx As IDataContext = DataContext.Instance()
                payments = ctx.ExecuteQuery(Of Models.Payment)(CommandType.StoredProcedure, "RIS_Payments_Get", portalId, account, provider, client, cat, done, sTerm, GetNull(CDate(sDate)), GetNull(CDate(eDate)), pageNumber, pageSize, orderBy)
            End Using
            Return payments
        End Function
