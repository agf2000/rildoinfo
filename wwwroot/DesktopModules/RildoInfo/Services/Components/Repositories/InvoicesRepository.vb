...
#End Region
    ...
    Public Function GetInvoices(portalId As String, providerId As String, personId As String, prodId As String, sDate As DateTime, eDate As DateTime, pageNumber As Integer, pageSize As Integer, orderBy As String) As IEnumerable(Of Models.Invoice) Implements IInvoicesRepository.GetInvoices
        Dim invoices As IEnumerable(Of Models.Invoice)

        Using ctx As IDataContext = DataContext.Instance()
            invoices = ctx.ExecuteQuery(Of Models.Invoice)(CommandType.StoredProcedure, "RIW_Invoices_Get", portalId, providerId, personId, prodId, GetNull(sDate), GetNull(eDate), pageNumber, pageSize, orderBy)
        End Using
        Return invoices
    End Function
    ...
    Public Function GetInvoice(invoiceId As Integer) As Models.Invoice Implements IInvoicesRepository.GetInvoice
        ...
        Using ctx As IDataContext = DataContext.Instance()
            invoice = ctx.ExecuteSingleOrDefault(Of Models.Invoice)(CommandType.StoredProcedure, "RIW_Invoice_Get", invoiceId)
        End Using
        Return invoice
    End Function
    ...
    Public Function GetInvoiceItems(invoiceId As Integer) As IEnumerable(Of Models.InvoiceItem) Implements IInvoicesRepository.GetInvoiceItems
        ...
        Using ctx As IDataContext = DataContext.Instance()
            invoiceItems = ctx.ExecuteQuery(Of Models.InvoiceItem)(CommandType.StoredProcedure, "RIW_InvoiceItems_Get", invoiceId)
        End Using
        Return invoiceItems
    End Function
    ...
    Public Sub RemoveInvoiceItems(invoiceId As Integer) Implements IInvoicesRepository.RemoveInvoiceItems
        Dim invoiceItems As IEnumerable(Of Models.InvoiceItem) = GetInvoiceItems(invoiceId)
